...
export function getBlockStyle(data: any): string {
	let styles = ''
	forEach(data, (key: any, value: any): any => {
		if (value) {
			styles += `${key}:${value};`
		}
	})
...
*/
function getHashtagRanges(blockText: string, hashtagConfig: any): Array<any> {
	const sections = []
	if (hashtagConfig) {
		let counter = 0
		let startIndex = 0
...
*/
function getSections(block: any, hashtagConfig: any): Array<any> {
	const sections = []
	let lastOffset = 0
	...
	sectionRanges = sectionRanges.sort((s1: any, s2: any) => s1.offset - s2.offset)
...
*/
function isAtomicEntityBlock(block: any): boolean {
...
*/
function getStyleArrayForBlock(block: any): any {
...
*/
export function getStylesAtOffset(inlineStyles: any, offset: number): any {
	const styles: any = {}
...
*/
export function sameStyleAsPrevious(inlineStyles: any, styles: Array<string>, index: number): boolean {
	let sameStyled = true
...
* Function returns html for text depending on inline style tags applicable to it.
*/
export function addStylePropertyMarkup(styles: any, text: string): string {
...
*/
function getEntityMarkup(entityMap: any, entityKey: number, text: string, customEntityTransform: Function): string {
	...
	if (entity.type === 'MENTION') {
		return `<a href="${entity.data.url}" class="wysiwyg-mention" data-mention data-value="${entity.data
			.value}">${text}</a>`
	}
	if (entity.type === 'LINK') {
	...
	if (entity.type === 'IMAGE') {
		return `<img src="${entity.data.src}" alt="${entity.data.alt}" style="float:${entity.data.alignment ||
			'none'};height: ${entity.data.height};width: ${entity.data.width}"/>`
	}
	if (entity.type === 'EMBEDDED_LINK') {
		return `<iframe width="${entity.data.width}" height="${entity.data.height}" src="${entity.data
...
*/
function getInlineStyleSections(block: any, styles: any, start: any, end: any): any {
	const styleSections: any = []
	const { text }: any = block
	if (text.length > 0) {
...
*/
function getStyleTagSectionMarkup(styleSection: any): string {
	const { styles, text } = styleSection
	let content = getSectionText(text)
	forEach(styles, (style: any, value: any) => {
...
*/
function getInlineStyleSectionMarkup(block: any, styleSection: any): string {
	...
	styleTagSections.forEach((stylePropertySection: any) => {
...
*/
function getSectionMarkup(block: any, entityMap: any, section: any, customEntityTransform: Function): string {
	...
	inlineStyleSections.forEach((styleSection: any) => {
...
export function getBlockMarkup(
	...
	if (isAtomicEntityBlock(block)) {
		blockHtml.push(getEntityMarkup(entityMap, block.entityRanges[0].key, '', customEntityTransform))
	} else {
